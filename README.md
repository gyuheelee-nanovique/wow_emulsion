...
`emulsion_bo_pipeline.py`는 double emulsion 현미경 이미지를 분석하고, 누적 실험 결과를 바탕으로 다음 실험 조건을 추천하는 스크립트입니다.
...
- `bo_history.csv` 하나에 실험 이력과 마지막 BO 추천을 함께 저장
...
python emulsion_bo_pipeline.py --experiments-root ./experiments
```

입력이 바뀌지 않았더라도 강제로 다시 분석하려면:

```bash
python emulsion_bo_pipeline.py --experiments-root ./experiments --force-reprocess
```

기존 분석 결과만 지우고 `experiments/` 입력은 유지한 채 처음부터 다시 돌리려면:

```bash
python emulsion_bo_pipeline.py --experiments-root ./experiments --reset-analysis
```
...
- 같은 `batch_id`이고 `condition.json` 값도 같으면 해당 실험은 건너뜁니다.
- 같은 `batch_id`라도 `condition.json` 값이 바뀌면 다시 분석하고 기존 row를 갱신합니다.
- 같은 입력으로 반복 실행해도 실험 이력이 중복 행으로 계속 쌓이지 않습니다.

`--reset-analysis`를 사용하면 아래 분석 산출물만 삭제한 뒤 전체 실험을 처음부터 다시 계산합니다.

- `bo_history.csv`
- `bo_visualization.png`
- 각 실험 폴더의 `droplets.csv`
- 각 실험 폴더의 `overlays/`
...
- `bo_history.csv`: 실험 이력 전체와 마지막 행의 BO 추천 조건을 함께 저장
- `droplets.csv`: 검출된 droplet별 측정값
- `bo_visualization.png`: BO 후보와 추천점을 보여주는 시각화
